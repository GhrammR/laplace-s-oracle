//! Deterministic integer math helpers for cross-platform orbital mechanics.
#![forbid(unsafe_code)]
...
#[inline]
pub fn sine_u8(phase: u8) -> i32 {
    SINE_TABLE[usize::from(phase)]
}

#[inline]
pub fn phase_u8(tick: u64, period: u16) -> u8 {
    let period_u64 = u64::from(period.max(1));
    let phase = ((tick % period_u64) * 256) / period_u64;
    u8::try_from(phase).unwrap_or(255)
}
